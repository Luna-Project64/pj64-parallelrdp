...
#include "retroarch/video_driver.h"
...
extern "C" retro_environment_t environ_cb;
extern "C" uint32_t rdram_size;
extern "C" GFX_INFO gfx;
...
{
	RDP::vulkan = (struct retro_hw_render_interface_vulkan*) retro_get_hw_render_interface();
	RDP::init();
}
...
extern "C" bool parallel_retro_init_vulkan(void)
{
	hw_render.version_major = VK_MAKE_VERSION(1, 0, 12);
	hw_render.context_reset = context_reset;
	hw_render.context_destroy = context_destroy;

	if (!retro_set_hw_render(&hw_render))
	{
		if (log_cb)
	...
	hw_context_negotiation.interface_version = 0;
	...
	if (retro_set_hw_render_context_negotiation_interface(&hw_context_negotiation))
	{
		if (log_cb)
